...
def crossValidate(data_set_parent):    
    ...
    for i in parts:  
        ...
        # Jump over some models
#         if '_0' in data_set_parent and 'part_0' in i:
#             pass
#         else:
#             continue
...
#                   os.path.join(data_set_parent, 'TwoCNN_over_sampling_'+str(keep_probability)+'_normalize_by_max_fill_by_0_stride_40_'+iname+'_as_test_'), '1'))
        ...
        model_dir = os.path.join(data_set_parent, prefix_useTwoLayers+'_kernel_'+
                                 str(keep_probability)+'_nooversamp_normalizebymean_fillby0_stride40_'+iname+'_as_test_')
...
#                  useTwoLayers, mean, zero, keep_probability, model_dir, '1', 1)
        ...
        CNNClass(path_training, path_testing, 21, 62, kernel1size, kernel2size, 1e-5, 20000, 
                 useTwoLayers, mean, zero, keep_probability, model_dir, '1', 1, divide_by_gcconent=True)
#     pool.close()
#     pool.join()
#     
if __name__ == '__main__':
    ...
    input_dir = 'D:/eclipse-workspace/CNN_CNV/data_ERBB2_stride40_split_to_training_test/training'
    input_dir = 'D:/eclipse-workspace/CNN_CNV/data_ERBB2_stride40_split_to_training_test/training'
    input_dir = 'D:/eclipse-workspace/CNN_CNV/data_bigger_training_added_strong_positive_stride_40_gccontent/mixed'
    #for i in range(9, 10):
    generate_fake = False
    path_testing = u"D:\eclipse-workspace\CNN_CNV\data_producing_soft_links_mats_stride_40\correct_label"
    model_dir = "D:/eclipse-workspace/CNN_CNV/data_bigger_training_added_strong_positive_stride_40_gccontent/5xCV_data_rand_9/one_kernel_0.5_nooversamp_normalizebymean_fillby0_stride40_part_3_as_test_"
    #model_dir = 'D:/eclipse-workspace/CNN_CNV/data_bigger_training_added_strong_positive_stride_40/5xCV_data_rand_6/Strong_positive_simpler_over_sampling_0.5_normalize_by_file_size_fill_by_0_stride_40_part_0_as_test_'
    useTwoLayers = False
    keep_probability = 0.5
    CNNClass(path_testing, path_testing, 21, 62, 5, 5, 1e-5, 0, 
                 useTwoLayers, mean, zero, keep_probability, model_dir, '1', 1, divide_by_gcconent=True)
    exit()
    
    for i in range(2,10):
        fold = 5
        output_dir = os.path.join(os.path.dirname(input_dir), str(fold)+'xCV_data_rand_'+str(i))
