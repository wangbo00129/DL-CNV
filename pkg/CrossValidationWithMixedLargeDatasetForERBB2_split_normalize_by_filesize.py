...
#                   os.path.join(data_set_parent, 'TwoCNN_over_sampling_'+str(keep_probability)+'_normalize_by_max_fill_by_0_stride_40_'+iname+'_as_test_'), '1'))
        ...
        model_dir = os.path.join(data_set_parent, prefix_useTwoLayers+'_kernel_'+
                                 str(keep_probability)+'_oversamp1_4fold_normalizebyfilesize_fillby0_stride25_'+iname+'_as_test_')
...
#                  useTwoLayers, mean, zero, keep_probability, model_dir, '1', 1)
        CNNClass(path_training, path_testing, 27, 24, kernel1size, kernel2size, 1e-5, 10000, 
                 useTwoLayers, 'file_size', zero, keep_probability, model_dir, '1', 1)
        
        path_testing = "D:/eclipse-workspace/CNN_CNV/data_ERBB2_stride40_split_to_training_test/testing"
#         CNNClass(path_training, path_testing, 27, 24, kernel1size, kernel2size, 1e-5, 0, 
#                  useTwoLayers, mean, zero, keep_probability, model_dir, '1', 1)
#     pool.close()
#     pool.join()
...
if __name__ == '__main__':
    ...
    input_dir = 'D:/eclipse-workspace/CNN_CNV/data_ERBB2_stride40_split_to_training_test_filesize/training'
    #for i in range(9, 10):
    generate_fake = False
    for i in range(0,10):
        fold = 10
