...
class DataSet(object):
    ...
    def readBatch(self, batch_num=10,shuffle=False):
        ...
        ys=[]
        sample_names=[]
        #log_i('_current_read: ',self.current_read)
        for _ in range(batch_num):
            f=self.files[self.current_read]
            # Features
            # log_i('current file: ', f)
...
size_conv1 = tf.placeholder(tf.int32)
...
size_conv2 = tf.placeholder(tf.int64)
...
FP_mat = tf.cast(tf.multiply(1-correct_mat, real_positive_mat), tf.float32)
...
#log_i(training_set.readBatch(15))
#log_i('__')
#log_i(testing_set.readBatch(10))
LOG_FILE_PATH = ''
...
def __main__(conv1_size, conv2_size):
    ...
    save_sess_path='d:\\eclipse-workspace\\CNN_CNV\\save_sess_stride_conv_size_'+str(conv1_size)+'_'+str(conv2_size)
    ...
    for step in range(10000):
        ...
        #log_i('final shape: ')
        #log_i(np.shape(train_xs)) 
        train_feed_dict = {x:train_xs,y:train_ys,keep_prob:0.6,size_conv1:conv1_size,size_conv2:conv2_size,minimizer_step:INIT_MINIMIZER_STEP_SIZE/log(step+3)}
        
        minimizer.run(feed_dict=train_feed_dict)
        if not step%100 == 0:
            continue
        
        log_i('training')
        accu = accuracy.eval(feed_dict={x:train_xs,y:train_ys,keep_prob:1.0,size_conv1:conv1_size,size_conv2:conv2_size})
        log_i('training accuracy: '+str(accu))
        
        #log_i(yhat.eval(feed_dict={x:train_xs,y:train_ys,keep_prob:1.0}))
        log_i('training cross entropy: ')
        log_i(cross_entropy.eval(feed_dict={x:train_xs,y:train_ys,keep_prob:1.0,size_conv1:conv1_size,size_conv2:conv2_size}))
        ...
        log_i(df_accu)
        log_i('testing accuracy: '+str(accu))
        log_i('testing recall_rate: '+str(recall_rate))
        log_i('testing false positive: '+str(false_negative_rate))
        log_i('batch done')
...
if __name__=='__main__':
    for conv1 in range(1,10):
        for conv2 in range(1, 10):
