import sys, os
import pickle
from EnumerateRefBins import RefGenomeReader
from GetGCContent import CGContentGetter
from numpy import mat
import glob

def seperateBinsAndCountReads(ref_file, file_reads, bin_size, stride_size, allow_residue=False, min_base_num=30, tolerance=0, output=None):
    ...
    aligner = CGContentGetter(ref_seqs, file_reads, output)
...
def readSampleInfo(ref_file, folder_reads, bin_size, stride_size, tolerance, label):
    ...
    output = os.path.join(folder_reads,str(label)+'_'+os.path.basename(folder_reads)+'.reads')
    read_counts = seperateBinsAndCountReads(ref_file, file_reads, bin_size, stride_size, tolerance, output)
