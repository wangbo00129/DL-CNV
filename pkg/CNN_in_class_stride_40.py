...
class CNNClass(object):
    ...
    def __init__(self, size_conv1, size_conv2):
        ...
        x=tf.placeholder(tf.float32, shape=[None, 62])
        
        x_2d=tf.reshape(x, [-1, 21, 62, 1])
        ...
        flattened=tf.reshape(out2, [-1, 6*16*64])
        w=tf.Variable(tf.truncated_normal(stddev=0.1, shape=[6*16*64, 1024]))
        ...
        save_sess_path='d:\\eclipse-workspace\\CNN_CNV\\stride_40_save_sess_stride_conv_size_'+str(size_conv1)+'_'+str(size_conv2)+'\\'
        ...
        if os.path.exists(save_sess_path):
            if len(os.listdir(save_sess_path))>2:
        ...
        training_set=DataSet('D:/eclipse-workspace/CNN_CNV/data_stride_40/training/')
        testing_set=DataSet('D:/eclipse-workspace/CNN_CNV/data_stride_40_strong_positive/stride_40')
        # Run.

        tick = time.time()
        for step in range(101):
            train_xs, train_ys, _ = training_set.readBatch(20, True)
            train_feed_dict = {x:train_xs,y:train_ys,keep_prob:0.6,minimizer_step:INIT_MINIMIZER_STEP_SIZE/log(step+3)}
...
if __name__=='__main__':
    for conv1 in range(4,8):
        for conv2 in range(1, 8):
