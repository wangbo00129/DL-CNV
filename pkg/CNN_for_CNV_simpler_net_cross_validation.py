...
class CNNClass(object):
    ...
    def __init__(self, path_training_set, path_testing_set, mat_shape_m, mat_shape_n, size_conv1, size_conv2, 
                 ...
                 over_sampling='1', over_sampling_fold=20, divide_by_gcconent=False):
            
        # The last item is for oversampling class. Leave None to ignore. 
        ...
        #writer = tf.summary.FileWriter(run_log, sess.graph)
                ...
                def generateAccuracyDataFrame(fed_xs, fed_ys, fed_names, keep_probability=1.0):
                    ...
                    false_positive_rate = float(sum(dict_test['FP']))/(sum(dict_test['FP'])+sum(dict_test['TN']))
                    precision = float(sum(dict_test['TP']))/(sum(dict_test['TP'])+sum(dict_test['FP']))
                    specificity = 1 - false_positive_rate
                    re_dataframe = df(dict_test, index=list(fed_names), columns=['predict', 'real', 'correct', 'TP', 'FN', 'TN', 'FP'])
                
                    return re_dataframe, accu, recall_rate, specificity, precision
            
        
                training_set=DataSet(path_training_set, DO_NORMALIZE_BY, REPLACE_ZEROS_WITH_SOME_VALUE, 
                                    over_sampling, over_sampling_fold, divide_by_gccontent=divide_by_gcconent)
                testing_set=DataSet(path_testing_set, DO_NORMALIZE_BY, REPLACE_ZEROS_WITH_SOME_VALUE, 
                                    load_fake = False, divide_by_gccontent=divide_by_gcconent)
                # Run.
                
                tick = time.time()
                for step in range(0, step_num+1):
                    if not step_num == 0:
                        # Only when the step_num bigger than 0 do training. 
                        train_xs, train_ys, train_names = training_set.readBatch(TRAINING_BATCH_NUM, True)
                    ...
                    if not step_num == 0:
                        ...
                        df_accu, accu, recall_rate, specificity, precision = generateAccuracyDataFrame(train_xs, train_ys, train_names)
                    ...
                    df_accu, accu, recall_rate, specificity, precision = generateAccuracyDataFrame(test_xs, test_ys, test_sample_names)
                    
                    self.log_i(df_accu, LOG_FILE_PATH)
                    self.log_i('testing_accuracy: '+str(accu), LOG_FILE_PATH)
                    self.log_i('testing_sensitivity: '+str(recall_rate), LOG_FILE_PATH)
                    self.log_i('testing_specificity: '+str(specificity), LOG_FILE_PATH)
                    self.log_i('testing_precision: '+str(precision), LOG_FILE_PATH)
