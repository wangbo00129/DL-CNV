...
def crossValidate(data_set_parent):    
    ...
    from pkg.CNN_for_CNV_simpler_net_cross_validation import __main__
    ...
    for i in parts: 
        ...
        path_testing = 'D:\\eclipse-workspace\\CNN_CNV\\data_stride_40_strong_positive\\stride_40'
...
#                   os.path.join(data_set_parent, 'TwoCNN_over_sampling_'+str(keep_probability)+'_normalize_by_max_fill_by_0_stride_40_'+iname+'_as_test_'), '1'))
        model_dir = os.path.join(data_set_parent, 'Strong_positive_simpler_over_sampling_'+str(keep_probability)+'_normalize_by_file_size_fill_by_0_stride_40_'+iname+'_as_test_')
        
        print(model_dir)
        __main__(path_training, path_testing, 21, 62, 5, 5, 
                1e-5, 10000, False, mean, zero, keep_probability,
                model_dir, '1')
        break
#     pool.close()
#     pool.join()
